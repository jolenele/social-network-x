...
export default function EditorPage() {
  ...
  // Send selected image to Vision endpoint
  async function analyzePhoto(photoUrl: string) {
    ...
    try {
      ...
      if (err instanceof Error && err.name === "AbortError") {
        setVisionError(
          "Vision API request timed out after 2 minutes. Please try again with a smaller image."
        );
      } else {
  ...
  async function saveToDatabase() {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to save transformation";
      setSaveError(errorMessage);

      let userMessage = errorMessage;
      if (errorMessage.includes("fetch")) {
        userMessage =
          "Failed to connect to server. Make sure the Express server is running on port 3001.";
      } else if (
        ...
        userMessage =
          "Server error. Check the Express server logs for details.";
      }
  ...
  async function generateWithGemini() {
    ...
    try {
      ...
      const prompt = buildHairModificationPrompt(color, style, visionValidation);
      ...
      if (err instanceof Error && err.name === "AbortError") {
        setGeminiError(
          "Image generation timed out after 5 minutes. Please try again - this can happen with complex transformations."
        );
      } else {
      ...
      <div className="w-64 bg-white border-r border-gray-200 shadow-sm flex flex-col h-screen overflow-y-auto">
        <div className="p-6 border-b border-gray-200">
          ...
          <p className="text-sm text-gray-600">
            Get ideas for your transformation
          </p>
        </div>
        ...
        <div className="flex-1 p-4 space-y-4">
            ...
            {showColors && (
              <div className="mt-2 space-y-1 animate-slide-down">
                <div className="grid grid-cols-2 gap-2 p-2">
                  {hairColors.map((colorItem, index) => (
                    <button
                      key={index}
          ...
          <div>
            ...
            {showHairstyles && (
              <div className="mt-2 space-y-1 animate-slide-down">
                <div className="grid grid-cols-2 gap-2 p-2">
                  {trendingHairstyles.map((styleItem, index) => (
                    <button
                      key={index}
              ...
              <div className="grid md:grid-cols-2 p-8 gap-8 relative">
                ...
                <div className="flex flex-col items-center md:border-r md:border-gray-200 md:pr-8">
                  ...
                  <p className="mt-2 text-sm text-gray-600 text-center max-w-md">
                    This represents your current look. Customize the options on
                    the right to see changes.
                  </p>
                </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <button
                      ...
                      disabled={
                        isVisionLoading ||
                        !selectedPhoto ||
                        visionValidation?.isValid === false ||
                        isGenerating
                      }
